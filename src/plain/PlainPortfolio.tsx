...
export function PlainPortfolio() {
  return <main className="plain-portfolio">
    <header className="plain-hero">
      ...
      <h1>{profile.name}</h1>
      <h2>{profile.role}</h2>
      {profile.bio && <p className="plain-intro">{profile.bio}</p>}
      <div className="plain-links">{profile.email && <a href={`mailto:${profile.email}`}>{profile.email}</a>}<a href={profile.socials.github}>GitHub</a><a href={profile.socials.linkedin}>LinkedIn</a></div>
    </header>
    ...
    <footer>Based in {profile.location}{profile.email && <> · <a href={`mailto:${profile.email}`}>Get in touch</a></>}</footer>
  </main>;
}
